...
/*   Updated: 2023/03/17 18:18:52 by jrouillo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	image_corner(t_data *data, int y, int x)
{
	if ((y == 0 && x == 0))
		mlx_put_image_to_window(data->mlx_ptr, data->win_ptr,
			data->img.w_topleft, x * PXL, y * PXL);
	else if ((y == 0 && x == data->width - 1))
		mlx_put_image_to_window(data->mlx_ptr, data->win_ptr,
			data->img.w_topright, x * PXL, y * PXL);
	else if ((y == data->height - 1 && x == 0))
		mlx_put_image_to_window(data->mlx_ptr, data->win_ptr,
			data->img.w_bottomleft, x * PXL, y * PXL);
	else if ((y == data->height - 1 && x == data->width - 1))
		mlx_put_image_to_window(data->mlx_ptr, data->win_ptr,
			data->img.w_bottomright, x * PXL, y * PXL);
}
